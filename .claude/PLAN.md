...
#### >> Next: Batch 4 — Responsive Fixes

#### Batch 4 — Responsive Fixes

- [ ] **17.14** Hero image mobile crop: on 375px–430px viewports, adjust `objectPosition` to keep the primary subject visible (e.g. `center 20%`). Test iPhone SE (375px) and iPhone 14 Pro (393px). File: `components/home/hero.tsx`.
- [ ] **17.15** Global Partners section mobile: reduce logo height from `h-12` to `h-8` below `md:`, reduce grid gap. Verify no horizontal overflow at 375px. File: `components/home/client-marquee.tsx`.

#### Batch 5 — New Features

- [ ] **17.16** News homepage section: review diff on `chore/plugin-fluff` branch, cherry-pick or manually port the news section component into `feat/ux-overhaul`. Insert into `app/page.tsx` at correct position. Verify section order and layout are intact.
- [ ] **17.17** Light/white mode theme: add a third theme option — light (white), base (navy), dark. Implement as a sliding pane or 3-way toggle. Default to base. Audit all components to ensure the white theme looks professional (not washed out). This touches `globals.css` and potentially every component's color tokens — do last.
